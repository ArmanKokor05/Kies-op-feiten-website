...
export default {
  /**
   * Fetches paginated chats/responses for a specific user
   *
   * @param userId - The ID of the user whose chats to retrieve
   * @param page - Page number to retrieve
   * @param size - Number of items per page
   * @returns Paginated response containing user chats
   * @throws HTTP error with status and message if request fails
   */
  async getDiscussionsOfUser(userId: number, page: number = 0, size: number = 20): Promise<PaginatedResponse<any>> {
    try {
